...
(define z (list 1 2 (list 3 4) (list 5 (list 7 8)) 9)) ; it seems it is failing, when it has list in the list in the list, but it isn't priority rigth now.
  ...
  (define (deep-rvrs-cycle items rvrsd-items other-items last-item)
    (if (and (null? items) (not (null? other-items)))
        ...
        (if (null? items)
            ...
            (if (pair? (car items))
                (if (null? (cdr items))
                    (deep-rvrs-cycle (car items) (cons (list) rvrsd-items) (cdr items) #t)
                    (deep-rvrs-cycle (car items) (cons (list) rvrsd-items) (cdr items) #f))
